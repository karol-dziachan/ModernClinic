...
export default function BookVisit(){
    ...
    useEffect(() => {
        ...
        if(date !== null && date !== undefined && time !== null && time !== undefined){
            console.log('time changed:', date, time);
            setTabData(prevTabData => {
                const updatedTabData = [...prevTabData];
                updatedTabData[1].Header = date && time ? `Wybrano date: ${date.toLocaleDateString('en-GB')}  w przedziale ${time.time}` : 'Data i czas wizyty';
                return updatedTabData;
            });
        }
        if(specialist !== null && specialist !== undefined){
            console.log('Specialist changed:', specialist);
            setTabData(prevTabData => {
                const updatedTabData = [...prevTabData];
                updatedTabData[2].Header = specialist ? `Wybrano specjaliste: ${specialist.name}` : 'Specjalista';
                return updatedTabData;
            });
        }
        setTabData(prevTabData => {
        ...
        removeTabAtIndex(2);
          ...
          addTabAtIndex(2, {
            ...
            Content: <SpecialistsForm setSpecialist={setSpecialist} dateId={time?.id} offerId={service?.id}/>
        });
        }
    ...
    const [tabData, setTabData] = useState([
        ...
        },
        {
            ...
            Content: <VisitDateTimePicker setDate={setDate} setTime={setTime}/>
        },
        {
            Header: 'Specjalista',
            Content:  <SpecialistsForm setSpecialist={setSpecialist} dateId={time?.id} offerId={service?.id}/>
    ...
    const sendForm = async () => {
        ...
        if(newErrors.length > 0){
            ...
            console.log('poczatek wysylania ', data)
             await apiClient.post('/api/visits/book-visit', data);
             console.log('zapisano wizyte:')
             setValidation([]);
             setIsSend(true);
            }catch(error){
                console.log('lapano blad')
